...
void initFunction() {
    ...
    gpio_init(BUTTON3_PIN); gpio_set_dir(BUTTON3_PIN, GPIO_IN); gpio_pull_up(BUTTON3_PIN);
...
[[noreturn]] void modbusFanTask(void *pvParameters) {
    ...
    while (true) {
        constexpr float desiredFanSpeed = 90.0f;
...
[[noreturn]] void modbusGmpTask(void *pvParameters) {
    ...
    while (true) {
        ...
        const float comp = gmpSensor->readCompensationT();
        const float measT = gmpSensor->readMeasuredT();

        if (!std::isnan(co2) && !std::isnan(comp) && !std::isnan(measT)) {
            char buf[128];
...
[[noreturn]] void I2cPressureSensorTask(void *pvParameters) {
    ...
    if (!pressureSensor->init()) debug("Failed to initialize SDP610 pressure sensor\n");
    while (true) {
        ...
        if (!std::isnan(pressure)) {
            ...
            debug(buf);
        } else debug("SDP610 pressure sensor read failed!\n");
...
[[noreturn]] int main() {
    ...
    xTaskCreate(debugTask, "DebugTask", 512, nullptr, TASK_LOW_PRIORITY, nullptr);
    xTaskCreate(watchDogTimer, "WatchDogTimer", 512, nullptr, WATCHDOG_PRIORITY, nullptr);
    xTaskCreate(modbusFanTask, "ModbusFanTask", 512, nullptr, TASK_HIGH_PRIORITY, nullptr);
    xTaskCreate(modbusGmpTask, "ModbusGmpTask", 512, nullptr, TASK_HIGH_PRIORITY, nullptr);
    // xTaskCreate(modbusHmpTask, "ModbusHmpTask", 512, nullptr, TASK_HIGH_PRIORITY, nullptr);
    // xTaskCreate(I2cPressureSensorTask, "PressureSensorTask", 512, nullptr, TASK_HIGH_PRIORITY, nullptr);
