...
public:
    ...
    /**
     * @brief Write multiple bytes from a buffer (handles page boundaries).
     */
    bool writeBlock(int addr, const uint8_t *buffer, size_t length);
     ...
     */
    bool verifyWrite(int addr, const uint8_t *expected, size_t length);

    /**
     * @brief Dump EEPROM memory contents for debugging.
     */
    void dumpMemory(int startAddr, int length);

private:
    void buildAddressBytes(int addr, uint8_t *out) const;
    bool writePageAligned(int addr, const uint8_t *buffer, size_t length);
};
